...
export const GET = async (event) => {
	...
	const res = await db
		.select({ image: organization.logo })
		.from(organization)
		.where(eq(organization.id, id))
		.limit(1);
	const img = res[0]?.image;
	if (!img) return json({ error: 'Organization not found or no logo set' }, { status: 404 });
...
// todo: set up authorization checks here (e.g. only org owners/admins can update the org avatar)
export const POST = async (event) => {
	...
	const res = await db
		...
		.where(eq(organization.id, id))
		.limit(1);
	if (!res[0]) return json({ error: 'Organization not found or no logo set' }, { status: 404 });

	// S3 object key
	const key = `organization/avatar/${id}`;
